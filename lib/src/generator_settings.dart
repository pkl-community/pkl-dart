...
Future<GeneratorSettings> createGeneratorSettings(
  ...
  if (projectPath ?? await findProjectDir(projectPath, fileSystem: fileSystem)
      case final project?) {
...
Future<String?> findProjectDir(String? projectDirFlag,
    {FileSystem? fileSystem}) async {
  ...
  await for (final file
      in fileSystem.directory(p.current).list(recursive: true)) {
    if (file is File) {
